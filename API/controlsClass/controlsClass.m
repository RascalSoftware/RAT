classdef controlsClass < matlab.mixin.CustomDisplay
    ...
    properties
        ...
        jumpProb = 0.5               % Jump probabilities
        pUnitGamma = 0.2
    ...
    methods
        ...
        function obj = set.jumpProb(obj,val)
            validateNumber(val, 'jumpProb must be a number');
            if (val < 0 || val > 1)
                throw(invalidValue('Jump probability jumpProb must be a fraction between 0 and 1'));
            end
            obj.jumpProb = val;
        end
    ...
    methods (Access = private)
        ...
        function obj = processDreamInput(obj, varargin)
            ...
            addParameter(p,'jumpProb', defaultJumpProb,  @isnumeric);
            ...
            % Parses the input or raises invalidOption error
            errorMsg = 'Only nSamples, nChains, jumpProb, pUnitGamma, boundHandling, adaptPCR, parallel, calcSldDuringFit, resamPars and display can be set while using the DREAM procedure';
            inputBlock = obj.parseInputs(p, properties, errorMsg);

            % Sets the values the for Dream parameters
            obj.nSamples = inputBlock.nSamples;
            obj.nChains = inputBlock.nChains;
            obj.jumpProb = inputBlock.jumpProb;

...
// Return Type  : void
//
void abeles_paraPoints(const coder::array<double, 2U> &x, const coder::array<
...
#pragma omp parallel for \
 ...
 private(quo,num,MI,beta,pimag,preal,rough,thick,blast,psub,N,preal_tmp,i,i1,reploop,re,nl,im,ar,quo_re,x_re,x_im,brm,i2,b_MI)
  ...
  for (int loop = 0; loop <= ub_loop; loop++) {
    thick = asin(x[loop] * 1.54 / 12.566370614359172);
    preal = cos(thick);
    ...
    for (reploop = 0; reploop < i; reploop++) {
      for (nl = 0; nl < i1; nl++) {
        ...
        if (num.im == 0.0) {
          ...
          preal = exp(num.re / 2.0);
          quo_re = num.im;
          ar = num.im;
          num.re = preal * (preal * cos(quo_re));
          num.im = preal * (preal * sin(ar));
        }
        ...
        if (x_im == 0.0) {
          ...
          preal = exp(x_re / 2.0);
          x_re = preal * (preal * cos(x_im));
          x_im = preal * (preal * sin(x_im));
        }
        ...
        if (ar == 0.0) {
          ...
          preal = exp(quo_re / 2.0);
          quo_re = preal * (preal * cos(ar));
          quo.re = quo_re;
          ar = preal * (preal * sin(ar));
          quo.im = ar;
        }
    ...
    if (quo.im == 0.0) {
      ...
      preal = exp(quo.re / 2.0);
      quo_re = quo.im;
      ar = quo.im;
      quo.re = preal * (preal * cos(quo_re));
      quo.im = preal * (preal * sin(ar));
    }

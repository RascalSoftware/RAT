...
void allocateLayersForContrast(const emlrtStack *sp, const emxArray_real_T
  *contrastLayers, const emxArray_cell_wrap_16 *outParameterisedLayers,
  emxArray_real_T *thisContrastLayers)
{
  ...
  emxEnsureCapacity_real_T(sp, thisContrastLayers, i, &ph_emlrtRTEI);
  n *= 5;
  for (i = 0; i < n; i++) {
  ...
  for (b_i = 0; b_i < n; b_i++) {
    ...
    for (i2 = 0; i2 < 5; i2++) {
      ...
      if ((i1 < 1) || (i1 > i3)) {
        emlrtDynamicBoundsCheckR2012b(i1, 1, i3, &fd_emlrtBCI, sp);
      }
      ...
      i3 = (int32_T)d - 1;
      if ((i3 < 0) || (i3 > i)) {
        emlrtDynamicBoundsCheckR2012b(i3, 0, i, &ed_emlrtBCI, sp);
      }

                     ...
                     ssub, const double repeats[2], emxArray_real_T *sldProfile)
{
  ...
  emxEnsureCapacity_real_T(Lays, i3);
  ...
  for (xoffset = 0; xoffset < loop_ub; xoffset++) {
    Lays->data[xoffset + Lays->size[0] * i3] = thisBox->data[xoffset];
  }

  emxFree_real_T(&thisBox);

  /* plot(x,Lays(:,(numberOfLayers*nrepeats)+1)) */
  i3 = (int)(2.0 * nrepeats + 2.0) - 1;
  loop_ub = Lays->size[0];
  for (xoffset = 0; xoffset < loop_ub; xoffset++) {
    Lays->data[xoffset + Lays->size[0] * i3] = airBox->data[xoffset];
  }
  ...
  if ((Lays->size[0] == 0) || (Lays->size[1] == 0)) {
    ...
    emxEnsureCapacity_real_T(SLD, i3);
  ...
  sldProfile->size[1] = 2;
  emxEnsureCapacity_real_T(sldProfile, i3);
  for (i3 = 0; i3 < loop_ub; i3++) {
    sldProfile->data[i3] = x->data[i3];
  }

  emxFree_real_T(&x);
  loop_ub = SLD->size[0];
  for (i3 = 0; i3 < loop_ub; i3++) {
    sldProfile->data[i3 + sldProfile->size[0]] = SLD->data[i3];
  }

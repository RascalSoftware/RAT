   ...
   case 0:
    {
      ...
      c_standardTF_stanLay_reflectivi(problemDef, &expl_temp, controls, problem,
        cell1, b_Simulation, cell3, b_layerSlds, b_sldProfiles, allLayers);
      ...
      i1 = cell1.size(0);
      for (b_i = 0; b_i < i1; b_i++) {
        reflectivity[b_i].f1.set_size(cell1[b_i].f1.size(0), 2);
        kstr = cell1[b_i].f1.size(0) * cell1[b_i].f1.size(1);
        for (i2 = 0; i2 < kstr; i2++) {
          reflectivity[b_i].f1[i2] = cell1[b_i].f1[i2];
        }
      }

      Simulation.set_size(b_Simulation.size(0));
      i1 = b_Simulation.size(0);
      for (b_i = 0; b_i < i1; b_i++) {
        Simulation[b_i].f1.set_size(b_Simulation[b_i].f1.size(0), 2);
        kstr = b_Simulation[b_i].f1.size(0) * b_Simulation[b_i].f1.size(1);
        for (i2 = 0; i2 < kstr; i2++) {
          Simulation[b_i].f1[i2] = b_Simulation[b_i].f1[i2];
        }
      }

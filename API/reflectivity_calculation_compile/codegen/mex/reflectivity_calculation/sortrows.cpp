...
static emlrtRTEInfo ej_emlrtRTEI = { 27,// lineNo
  1,                                   // colNo
  "sortrows",                          // fName
  "/usr/local/MATLAB/R2020a/toolbox/eml/lib/matlab/datafun/sortrows.m"// pName
};

static emlrtRTEInfo fj_emlrtRTEI = { 86,// lineNo
  15,                                  // colNo
  "sortIdx",                           // fName
...
void sortrows(const emlrtStack *sp, coder::array<real_T, 2U> &y)
{
  ...
  st.site = &ve_emlrtRSI;
  n = y.size(0) + 1;
  idx.set_size((&ej_emlrtRTEI), (&st), y.size(0));
  i2 = y.size(0);
  for (j = 0; j < i2; j++) {
    idx[j] = 0;
  }
  ...
  if (y.size(0) == 0) {
    ...
    iwork.set_size((&fj_emlrtRTEI), (&b_st), y.size(0));
    i2 = y.size(0) - 1;
  ...
  st.site = &we_emlrtRSI;
  i2 = y.size(0);
  ycol.set_size((&gj_emlrtRTEI), (&st), y.size(0));

classdef controlsDef < matlab.mixin.CustomDisplay
    ...
    methods
        function obj = set.parallel(obj,val)
            if ~strcmpi(val,{'single','parallelTF', 'parallelAlg'})
                error('Type must be ''single'', ''parallelTF'', ''parallelAlg'' ');
            end
        ...
        function obj = set.procedure(obj,val)
            if ~strcmpi(val,{'calculate','simplex','DE','NS'})
                error('Type must be ''calculate'', ''simplex'', ''DE'' or ''NS'' ');
            end
        ...
        function groups = getPropertyGroups(obj)
            ...
            if isscalar(obj)
                ...
                elseif strcmpi(obj.procedure,'simplex')
                    dispPropList = rmfield(masterPropList,[DECell NSCell]);
                elseif strcmpi(obj.procedure,'DE')
                    dispPropList = rmfield(masterPropList,[simplexCell NSCell]);
                elseif strcmpi(obj.procedure,'NS')
                    dispPropList = rmfield(masterPropList,[simplexCell DECell]);    
                end

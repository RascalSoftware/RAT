...
static emlrtRTEInfo h_emlrtRTEI = { 73,/* lineNo */
  9,                                   /* colNo */
  "matlabEngineCaller_customLayers",   /* fName */
  "/home/arwel/Documents/RascalDev/RAT/Sandbox/matlabEngineCaller/matlabEngineCaller_customLayers.m"/* pName */
};
...
void matlabEngineCaller_customLayers(const emlrtStack *sp, const emxArray_real_T
  *params, real_T contrast, const emxArray_char_T *funcName, const
  emxArray_char_T *funcPath, const emxArray_real_T *bulkIn, const
  emxArray_real_T *bulkOut, emxArray_real_T *output, real_T *sRough)
{
  ...
  emxArray_real_T *b_bulkIn;
  emxArray_real_T *b_bulkOut;
  real_T outp[300];
  real_T nLayers;
  uint32_T rowCount;
  int32_T colCount;
  ...
  *sRough = params->data[0];
  ...
  emxInit_real_T(sp, &b_params, 2, &i_emlrtRTEI, true);
  ...
  emxInit_real_T(sp, &b_bulkIn, 2, &i_emlrtRTEI, true);
  i = b_bulkIn->size[0] * b_bulkIn->size[1];
  b_bulkIn->size[0] = 1;
  b_bulkIn->size[1] = bulkIn->size[1];
  emxEnsureCapacity_real_T(sp, b_bulkIn, i, &e_emlrtRTEI);
  loop_ub = bulkIn->size[0] * bulkIn->size[1];
  for (i = 0; i < loop_ub; i++) {
    b_bulkIn->data[i] = bulkIn->data[i];
  }

  emxInit_real_T(sp, &b_bulkOut, 2, &i_emlrtRTEI, true);
  i = b_bulkOut->size[0] * b_bulkOut->size[1];
  b_bulkOut->size[0] = 1;
  b_bulkOut->size[1] = bulkOut->size[1];
  emxEnsureCapacity_real_T(sp, b_bulkOut, i, &f_emlrtRTEI);
  loop_ub = bulkOut->size[0] * bulkOut->size[1];
  for (i = 0; i < loop_ub; i++) {
    b_bulkOut->data[i] = bulkOut->data[i];
  }
  ...
  matlabCallFun(&b_params->data[0], (int16_T)i, &funName->data[0],
                &pathCall->data[0], &b_bulkIn->data[0], &b_bulkOut->data[0],
                contrast, outp, &nLayers);
  rowCount = 1U;
  colCount = 1;

  /* reshape the output to [layers * 3] array */
  emxFree_real_T(&b_bulkOut);
  emxFree_real_T(&b_bulkIn);
  ...
  if (nLayers >= 1.0) {
    ...
    output->size[1] = 3;
    emxEnsureCapacity_real_T(sp, output, loop_ub, &h_emlrtRTEI);
    ...
    output->size[0] = 1;
    output->size[1] = 3;
    emxEnsureCapacity_real_T(sp, output, i, &g_emlrtRTEI);
    output->data[0] = 1.0;
    output->data[1] = 1.0;
    output->data[2] = 1.0;
    *sRough = 1.0;

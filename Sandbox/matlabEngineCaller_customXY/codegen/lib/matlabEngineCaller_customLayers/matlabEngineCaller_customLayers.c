...
void matlabEngineCaller_customLayers(const double params[18], double contrast,
  const char funcName[33], const char funcPath[71], double bulkIn, double
  bulkOut, emxArray_real_T *output, double *sRough)
{
  ...
  *sRough = params[0];
  ...
  cv2[2] = ' ';
  cv2[3] = '\'';
  memcpy(&cv2[4], &funcPath[0], 71U * sizeof(char));
  cv2[75] = '\'';
  cv2[76] = ';';
  memcpy(&b_params[0], &params[0], 18U * sizeof(double));

...
static emlrtBCInfo emlrtBCI = { -1,    /* iFirst */
  ...
  "/home/arwel/Documents/RascalDev/RAT/Sandbox/matlabEngineCaller_customXY/matlabEngineCaller_customXY.m",/* pName */
  0                                    /* checkKind */
};
  ...
  *output, real_T *sRough)
{
  ...
  *sRough = params->data[0];
  ...
  matlabCallFun(&b_params->data[0], (int16_T)i, &funName->data[0],
                &pathCall->data[0], bulkIn, bulkOut, contrast, outp, &nPoints);
  rowCount = 1U;
  colCount = 1;
  ...
  if (nPoints >= 1.0) {
    i = (int32_T)muDoubleScalarFloor(nPoints);
    if (nPoints != i) {
      emlrtIntegerCheckR2012b(nPoints, &emlrtDCI, sp);
    }

    loop_ub = output->size[0] * output->size[1];
    output->size[0] = (int32_T)nPoints;
    output->size[1] = 2;
    emxEnsureCapacity_real_T(sp, output, loop_ub, &f_emlrtRTEI);
    if ((int32_T)nPoints != i) {
      emlrtIntegerCheckR2012b(nPoints, &b_emlrtDCI, sp);
    }

    loop_ub = (int32_T)nPoints << 1;
    for (i = 0; i < loop_ub; i++) {
      output->data[i] = 0.0;
    }

    u = (uint32_T)nPoints * 2U;
    i = (int32_T)u;
    ...
    for (b_i = 0; b_i < i; b_i++) {
      ...
      if ((loop_ub < 1) || (loop_ub > 15000)) {
        emlrtDynamicBoundsCheckR2012b(loop_ub, 1, 15000, &b_emlrtBCI, sp);
      }
      ...
      rowCount++;
      if (rowCount == (uint32_T)nPoints + 1U) {
        rowCount = 1U;
        colCount++;
      }

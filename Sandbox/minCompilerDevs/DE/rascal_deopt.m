...
FVr_bestmem   = zeros(1,2);% best population member ever
...
%-----Select which vectors are allowed to enter the new population------------
  for k=1:I_NP
      ...
      S_tempval = fname(FM_ui(k,:),problem);   % check cost of competitor
      I_nfeval  = I_nfeval + 1;
      if (left_win(S_tempval,S_val(k)) == 1)   
         ...
         if (left_win(S_tempval,S_bestval) == 1)   
            ...
            FVr_bestmem = FM_ui(k,[1 2]);                 % new best parameter vector ever
         end
      end
   end % for k = 1:NP
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0)
     if ((rem(I_iter,I_refresh) == 0) | I_iter == 1)
       %addInfoText(sprintf('Iteration: %d,  Best: %f,  F_weight: %f,  F_CR: %f,  I_NP: %d\n',I_iter,S_bestval.FVr_oa(1),F_weight,F_CR,I_NP));
       disp(S_bestval);
       %var(FM_pop)
%        format long e;
%        for n=1:I_D
%           addInfoText(sprintf('best(%d) = %g\n',n,FVr_bestmem(n)));
%        end
        if (I_plotting == 1)

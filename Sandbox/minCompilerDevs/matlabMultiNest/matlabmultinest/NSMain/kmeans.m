...
%
if nargin < 2
    disp('At least two input arguments required.'); return;
end

% n points in p dimensional space
[n, p] = size(X);
Xsort = []; Xord = []; normC = zeros(k,1);
...
if ischar(distance)
    ...
    if length(i) > 1
        fprintf('Ambiguous ''distance'' parameter value:  %s.', distance); return;
    elseif isempty(i)
        fprintf('Unknown ''distance'' parameter value:  %s.', distance); return;
    end
...
if ischar(start)
    ...
    i = find(strncmp(lower(distance), startNames,length(lower(distance))));
    if length(i) > 1
        fprintf('Ambiguous ''start'' parameter value:  %s.', start); return;
    elseif isempty(i)
        fprintf('Unknown ''start'' parameter value:  %s.', start); return;
    elseif isempty(k)
    ...
    if strcmp(start, 'uniform')
        if strcmp(distance, 'hamming')
            disp('Hamming distance cannot be initialized with uniform random values.'); return;
        end
        Xmins = min(X,1);
        Xmaxs = max(X,1);
    end
...
if ischar(emptyact)
    ...
    if length(i) > 1
        fprintf('Ambiguous ''emptyaction'' parameter value:  %s.', emptyact); return;
    elseif isempty(i)
        fprintf('Unknown ''emptyaction'' parameter value:  %s.', emptyact); return;
    end
...
if ischar(display)
    ...
    if length(i) > 1
        fprintf('Ambiguous ''display'' parameter value:  %s.', display); return;
    elseif isempty(i)
        fprintf('Unknown ''display'' parameter value:  %s.', display); return;
    end
    display = i-1;
else
...
for rep = 1:reps
    ...
    while true
        ...
        if ~isempty(empties)
            switch emptyact
            case 'error'
                fprintf('Empty cluster created at iteration %d.',iter); return;
            case 'drop'
    ...
    while iter < maxit
        ...
        case {'cosine','correlation'}
            ...
            if any(normC < eps) % small relative to unit-length data points
                fprintf('Zero cluster centroid created at iteration %d.',iter); return;
            end
        ...
        if moved <= lastmoved
            iter = iter + 1;
            if display > 2 % 'iter'
                fsprintf(dispfmt,iter,2,nummoved,totsumD);
            end
            if iter >= maxit, break; end
            nummoved = 0;
        end
    ...
    if (~converged) & (display > 0)
        fprintf('Failed to converge in %d iterations.', maxit);
    end
    ...
    if display > 1 % 'final' or 'iter'
        fprintf('%d iterations, total sum of distances = %g',iter,totsumD);
    end
...
case {'cosine','correlation'}
    ...
    if any(normC < eps) % small relative to unit-length data points
        fprintf('Zero cluster centroid created at iteration %d.',iter);
    end
    % This can be done without a loop, but the loop saves memory allocations
    for i = clusts
        D(:,i) = 1 - (X * C(i,:)') ./ normC(i);

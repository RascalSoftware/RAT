...
if mod(nargs,2)~=0
    emsg = fprintf('Wrong number of arguments.');
else
    % Process name/value pairs
    for j=1:2:nargs
        pname = varargin{j};
        if ~ischar(pname)
            emsg = fprintf('Parameter name must be text.');
            break;
        end
        i = find(strncmp(lower(pname),pnames,length(pnames)))
        if isempty(i)
            ...
            if nargout > nparams+1
                ...
                emsg = fprintf('Invalid parameter name:  %s.',pname);
                break;
            end
        elseif length(i)>1
            emsg = fprintf('Ambiguous parameter name:  %s.',pname);
            break;
        else

...
// Return Type  : void
//
void abeles_single(const coder::array<double, 1U> &x, const coder::array<double,
    ...
    for (int reploop = 0; reploop < i1; reploop++) {
      for (int nl = 0; nl < i2; nl++) {
        ...
        if (rij_im == 0.0) {
          ...
          theta = std::exp(rij_re / 2.0);
          rij_re = theta * (theta * std::cos(rij_im));
          rij_im = theta * (theta * std::sin(rij_im));
        }
        ...
        if (x_im == 0.0) {
          ...
          theta = std::exp(x_re / 2.0);
          x_re = theta * (theta * std::cos(x_im));
          x_im = theta * (theta * std::sin(x_im));
        }
        ...
        if (pair_im == 0.0) {
          ...
          theta = std::exp(pair_re / 2.0);
          pair_re = theta * (theta * std::cos(pair_im));
          pair_im = theta * (theta * std::sin(pair_im));
        }

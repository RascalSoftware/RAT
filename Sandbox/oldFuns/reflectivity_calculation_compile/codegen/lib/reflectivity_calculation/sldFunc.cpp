             ...
             1U> &sldVal)
{
  ...
  if (where.size(0) != 0) {
    ...
    deltaY = (x - SLD[static_cast<int>(where[where.size(0) - 1]) - 1]) * (std::
      abs(deltaY_tmp - SLD[(static_cast<int>(where[where.size(0) - 1]) +
      SLD.size(0)) - 1]) / (SLD[nx] - SLD[static_cast<int>(where[where.size(0) -
      1]) - 1]));

...
#include "callReflectivity_rtwutil.h"

/* Function Definitions */
void paratt(const emxArray_real_T *x, const double sld_data[], const int
            sld_size[2], double nbair, double nbsub, double nrepeats, double
            rfinal, double layers, double points, emxArray_real_T *out)
{
  double snair;
  double snsub;
  int i3;
  ...
  i4 = out->size[0];
  out->size[0] = i3;
  emxEnsureCapacity_real_T(out, i4);
  for (loop = 0; loop < i3; loop++) {
    ...
    MI[1].im = 0.0;
    i4 = (int)nrepeats;
    for (reploop = 0; reploop < i4; reploop++) {
      i5 = (int)layers;
      for (nl = 0; nl < i5; nl++) {
        rough = sld_data[nl + (sld_size[0] << 1)];
        theta = 1.0 - sld_data[nl + sld_size[0]] * 0.377451863036739;
        ...
        pair_re = pimag.re;
        pair_im = pimag.im;
        blast_re = 4.0799904592075231 * sld_data[nl] * pimag.re;
        blast_im = 4.0799904592075231 * sld_data[nl] * pimag.im;
        for (i6 = 0; i6 < 2; i6++) {
          b_MI[i6].re = (MI[i6].re * N[0].re - MI[i6].im * N[0].im) + (MI[i6 + 2]
            .re * N[1].re - MI[i6 + 2].im * N[1].im);
          b_MI[i6].im = (MI[i6].re * N[0].im + MI[i6].im * N[0].re) + (MI[i6 + 2]
            .re * N[1].im + MI[i6 + 2].im * N[1].re);
          b_MI[i6 + 2].re = (MI[i6].re * N[2].re - MI[i6].im * N[2].im) + (MI[i6
            + 2].re * N[3].re - MI[i6 + 2].im * N[3].im);
          b_MI[i6 + 2].im = (MI[i6].re * N[2].im + MI[i6].im * N[2].re) + (MI[i6
            + 2].re * N[3].im + MI[i6 + 2].im * N[3].re);
        }
    ...
    N[2].im = pair_re * z_im + pair_im * z_re;
    for (i4 = 0; i4 < 2; i4++) {
      b_MI[i4].re = (MI[i4].re * N[0].re - MI[i4].im * N[0].im) + (MI[i4 + 2].re
        * N[1].re - MI[i4 + 2].im * N[1].im);
      b_MI[i4].im = (MI[i4].re * N[0].im + MI[i4].im * N[0].re) + (MI[i4 + 2].re
        * N[1].im + MI[i4 + 2].im * N[1].re);
      b_MI[i4 + 2].re = (MI[i4].re * N[2].re - MI[i4].im * N[2].im) + (MI[i4 + 2]
        .re * N[3].re - MI[i4 + 2].im * N[3].im);
      b_MI[i4 + 2].im = (MI[i4].re * N[2].im + MI[i4].im * N[2].re) + (MI[i4 + 2]
        .re * N[3].im + MI[i4 + 2].im * N[3].re);
    }

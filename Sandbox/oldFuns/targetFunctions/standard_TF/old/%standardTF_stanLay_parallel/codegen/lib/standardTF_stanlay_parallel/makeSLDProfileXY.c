...
void makeSLDProfileXY(double nbair, double nbsub, double ssub, const double
                      layers_data[], const int layers_size[2], double
                      numberOfLayers, double nrepeats, emxArray_real_T *out)
{
  emxArray_real_T *x;
  emxArray_real_T *airBox;
  emxArray_real_T *SLD;
  int i3;
  double b_layers_data[2];
  int vstride;
  int b_layers_size[1];
  emxArray_real_T c_layers_data;
  int mtmp;
  int b_mtmp;
  double layerThicks;
  emxArray_real_T *r0;
  ...
  if (numberOfLayers > 0.0) {
    vstride = layers_size[0];
    b_layers_size[0] = layers_size[0];
    for (i3 = 0; i3 < vstride; i3++) {
      b_layers_data[i3] = layers_data[i3];
    }

    c_layers_data.data = (double *)&b_layers_data;
    c_layers_data.size = (int *)&b_layers_size;
    c_layers_data.allocatedSize = 2;
    c_layers_data.numDimensions = 1;
    c_layers_data.canFreeData = false;
    layerThicks = sum(&c_layers_data);
    ...
    nextLayRough = layers_data[layers_size[0] << 1];
    ...
    /*      throw(ME); */
    /*  end */
    layerThicks = 1.4142135623730951 * layers_data[layers_size[0] << 1];
    bFactor = 1.4142135623730951 * layers_data[layers_size[0] << 1];
    ...
    while (mtmp <= (int)nrepeats - 1) {
      for (i = 0; i < (int)numberOfLayers; i++) {
        if (1.0 + (double)i < numberOfLayers) {
          nextLayRough = layers_data[1 + (layers_size[0] << 1)];
        ...
        layerThicks += layers_data[i] / 2.0;
        asymconvstep(x, layers_data[i], layerThicks, layers_data[i +
                     (layers_size[0] << 1)], nextLayRough, layers_data[i +
                     layers_size[0]], airBox);
        vstride = Lays->size[0];
        i3 = r2->size[0];
        r2->size[0] = vstride;

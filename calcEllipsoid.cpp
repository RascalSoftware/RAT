...
namespace RAT
{
                     ...
                     double VE_data[], int VE_size[2])
  {
    ...
    VE_tmp = static_cast<double>(u.size(1)) / 2.0 + 1.0;
    coder::b_gamma(&VE_tmp);
    ...
    for (int b_i{0}; b_i < i; b_i++) {
      ...
      for (i1 = 0; i1 < loop_ub; i1++) {
        b_u[i1] = u[b_i + u.size(0) * i1] - mu[i1];
      }
      ...
      loop_ub = r.size(1);
      for (i1 = 0; i1 < loop_ub; i1++) {
        d += r[i1] * (u[b_i + u.size(0) * i1] - mu[i1]);
      }
    ...
    VE_size[0] = 1;
    VE_size[1] = 1;
    VE_tmp = rt_powd_snf(3.1415926535897931, static_cast<double>(u.size(1)) /
                         2.0) / VE_tmp * std::sqrt(coder::det(y));
    VE_data[0] = VE_tmp;

    //  expand volume of bounding ellipsoid to VS if necessary
    fV_data = 1.0;
    const_data = (VE_tmp < VS);
    ...
    if (coder::internal::c_ifWhileCond(b_const_data)) {
      ...
      fV_data = std::log(VS / VE_tmp) * (2.0 / static_cast<double>(u.size(1)));
    ...
    for (i = 0; i < loop_ub; i++) {
      b_loop_ub = C.size(0);
      for (i1 = 0; i1 < b_loop_ub; i1++) {
        B[i1 + B.size(0) * i] = VE_tmp * C[i1 + C.size(0) * i];
      }
    }
  }
}

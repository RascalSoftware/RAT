...
static void e_emlrt_marshallIn(const emlrtStack *sp, const mxArray *this_data,
  ...
  const char_T *identifier, emxArray_real_T *y)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  f_emlrt_marshallIn(sp, emlrtAlias(this_data), &thisId, y);
...
void callReflectivity_api(const mxArray *prhs[9], const mxArray *plhs[2])
{
  ...
  emxInit_real_T(&st, &this_data, 2, true);
  emxInit_real_T(&st, &layers, 2, true);
  emxInit_real_T(&st, &reflectivity, 2, true);
  ...
  e_emlrt_marshallIn(&st, emlrtAlias(prhs[4]), "this_data", this_data);
  e_emlrt_marshallIn(&st, emlrtAlias(prhs[5]), "layers", layers);
  ...
  callReflectivity(nbairs, nbsubs, *simLimits, *repeatLayers, this_data, layers,
                   ssubs, backgrounds, res, reflectivity, Simulation);

  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(reflectivity);
  plhs[1] = emlrt_marshallOut(Simulation);
  Simulation->canFreeData = false;
  emxFree_real_T(&Simulation);
  reflectivity->canFreeData = false;
  emxFree_real_T(&reflectivity);
  layers->canFreeData = false;
  emxFree_real_T(&layers);
  this_data->canFreeData = false;
  emxFree_real_T(&this_data);

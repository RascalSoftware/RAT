...
#pragma omp for nowait
    ...
    for (loop = 0; loop <= ub_loop; loop++) {
      ...
      try {
        i = loop + 1;
        if ((i < 1) || (i > x.size(0))) {
          emlrtDynamicBoundsCheckR2012b(i, 1, x.size(0), &ah_emlrtBCI, &st);
        }
        ...
        blast.im = 0.0;
        if (1 > sld.size(0)) {
          emlrtDynamicBoundsCheckR2012b(1, 1, sld.size(0), &yg_emlrtBCI, &st);
        }
        ...
        for (reploop = 0; reploop < i; reploop++) {
          for (nl = 0; nl < i1; nl++) {
            ...
            if ((i2 < 1) || (i2 > sld.size(0))) {
              emlrtDynamicBoundsCheckR2012b(i2, 1, sld.size(0), &bh_emlrtBCI,
                &st);
            }
        ...
        if ((i < 1) || (i > out.size(0))) {
          emlrtDynamicBoundsCheckR2012b(i, 1, out.size(0), &eh_emlrtBCI, &st);
        }

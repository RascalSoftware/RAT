...
static emlrtRTEInfo tg_emlrtRTEI = { 9,/* lineNo */
  20,                                  /* colNo */
  "allocateParamsToLayers",            /* fName */
  "/home/arwel/Documents/RascalDev/RAT/targetFunctions/common/groupLayers/allocateParamsToLayers.m"/* pName */
};
...
void allocateParamsToLayers(const emlrtStack *sp, const emxArray_real_T *params,
  const emxArray_cell_wrap_3 *layersDetails, emxArray_cell_wrap_18 *outLayers)
{
  int32_T i;
  ...
  emxEnsureCapacity_cell_wrap_18(sp, outLayers, i, &tg_emlrtRTEI);
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    for (n = 0; n <= i1 - 2; n++) {
      ...
      if (covrtLogIf(&emlrtCoverageInstance, 6U, 0U, 0, covrtLogMcdc
                     (&emlrtCoverageInstance, 6U, 0U, 0, !covrtLogCond
                      (&emlrtCoverageInstance, 6U, 0U, 0, muDoubleScalarIsNaN
        ...
        if ((i2 < 1) || (i2 > params->size[1])) {
          emlrtDynamicBoundsCheckR2012b(i2, 1, params->size[1], &wc_emlrtBCI, sp);
        }
    ...
    if (layersDetails->data[b_i].f1.size[1] < 1) {
      emlrtDynamicBoundsCheckR2012b(layersDetails->data[b_i].f1.size[1], 1,
        layersDetails->data[b_i].f1.size[1], &tc_emlrtBCI, sp);
    }
    ...
    for (i2 = 0; i2 < 5; i2++) {
      if (b_i > i1) {
        emlrtDynamicBoundsCheckR2012b(b_i, 0, i1, &vc_emlrtBCI, sp);
      }

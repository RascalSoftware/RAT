...
static emlrtRSInfo gd_emlrtRSI = { 18, /* lineNo */
...
static emlrtDCInfo jb_emlrtDCI = { 23, /* lineNo */
  16,                                  /* colNo */
  "resampleLayers",                    /* fName */
  "/home/arwel/Documents/RascalDev/RAT/targetFunctions/common/resampleLayers/resampleLayers.m",/* pName */
  4                                    /* checkKind */
...
static emlrtRTEInfo ai_emlrtRTEI = { 23,/* lineNo */
  1,                                   /* colNo */
  "resampleLayers",                    /* fName */
  "/home/arwel/Documents/RascalDev/RAT/targetFunctions/common/resampleLayers/resampleLayers.m"/* pName */
};
                    ...
                    emxArray_real_T *newSLD)
{
  ...
  if (1 > sldProfile->size[0]) {
    emlrtDynamicBoundsCheckR2012b(1, 1, sldProfile->size[0], &xd_emlrtBCI, sp);
  }
  ...
  emxInit_real_T(sp, &out_f1, 2, &yh_emlrtRTEI, true);
  emxInitStruct_cell_20(sp, &expl_temp, &yh_emlrtRTEI, true);
  ...
  out_f1->size[1] = 2;
  emxEnsureCapacity_real_T(sp, out_f1, i, &yh_emlrtRTEI);
  loop_ub = expl_temp.f1->size[0] * expl_temp.f1->size[1];
  for (i = 0; i < loop_ub; i++) {
    out_f1->data[i] = expl_temp.f1->data[i];
  }

  emxFreeStruct_cell_20(&expl_temp);
  if (out_f1->size[0] == 0) {
    loop_ub = -1;
  ...
  if (loop_ub < 0) {
    emlrtNonNegativeCheckR2012b(loop_ub, &jb_emlrtDCI, sp);
  }

  i = newSLD->size[0] * newSLD->size[1];
  newSLD->size[0] = loop_ub;
  newSLD->size[1] = 3;
  emxEnsureCapacity_real_T(sp, newSLD, i, &ai_emlrtRTEI);
  loop_ub *= 3;
  for (i = 0; i < loop_ub; i++) {
    newSLD->data[i] = 0.0;
  }
  ...
  for (n = 0; n <= loop_ub; n++) {
    ...
    if ((i < 1) || (i > out_f1->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, out_f1->size[0], &vd_emlrtBCI, sp);
    }

    i = (int32_T)(((real_T)n + 1.0) + 1.0);
    if ((i < 1) || (i > out_f1->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, out_f1->size[0], &ud_emlrtBCI, sp);
    }

    i1 = n + 1;
    if ((i1 < 1) || (i1 > out_f1->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i1, 1, out_f1->size[0], &td_emlrtBCI, sp);
    }

    if (i > out_f1->size[0]) {
      emlrtDynamicBoundsCheckR2012b(i, 1, out_f1->size[0], &sd_emlrtBCI, sp);
    }

    thisLayRho = out_f1->data[(n + out_f1->size[0]) + 1];
    if (covrtLogIf(&emlrtCoverageInstance, 14U, 0U, 0, thisLayRho > out_f1->
    ...
    if ((i < 1) || (i > newSLD->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, newSLD->size[0], &rd_emlrtBCI, sp);
    }

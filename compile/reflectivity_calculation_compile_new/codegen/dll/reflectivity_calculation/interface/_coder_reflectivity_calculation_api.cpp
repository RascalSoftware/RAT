...
static const mxArray *emlrt_marshallOut(const cell_9 *u)
{
  ...
  b_y = nullptr;
  emlrtAssign(&b_y, emlrtCreateCellArrayR2014a(
                        1, ((coder::array<real_T, 1U> *)&u->f6)->size()));
  for (int32_T i{0}; i < u->f6.size(0); i++) {
    c_y = nullptr;
    m = emlrtCreateDoubleScalar(u->f6[i]);
    emlrtAssign(&c_y, m);
...
static const mxArray *emlrt_marshallOut(const struct4_T *u)
{
  ...
  emlrtSetFieldR2017b(b_y, 0, (const char_T *)"all_chis",
                      emlrt_marshallOut(u->calculations.all_chis), 0);
  c_y = nullptr;
...
static const mxArray *emlrt_marshallOut(const coder::array<cell_wrap_7, 1U> &u)
{
  ...
  for (int32_T i{0}; i < u.size(0); i++) {
    ...
    pData = emlrtMxGetPr(m);
    i1 = 0;
    for (b_i = 0; b_i < u[i].f1.size(0); b_i++) {
      pData[i1] = u[i].f1[b_i];
      i1++;
    }
    for (b_i = 0; b_i < u[i].f1.size(0); b_i++) {
      pData[i1] = u[i].f1[b_i + u[i].f1.size(0)];
      i1++;
    }
    emlrtAssign(&b_y, m);
...
static const mxArray *emlrt_marshallOut(const coder::array<cell_wrap_8, 1U> &u)
{
  ...
  for (int32_T i{0}; i < u.size(0); i++) {
    ...
    for (b_i = 0; b_i < u[i].f1.size(0); b_i++) {
      pData[i1] = u[i].f1[b_i + u[i].f1.size(0) * 2];
      i1++;
    }
    emlrtAssign(&b_y, m);
    emlrtSetCell(y, i, b_y);

...
static emlrtRSInfo ug_emlrtRSI = { 31, /* lineNo */
  "abeles_paraPoints",                 /* fcnName */
  "/home/sp1115144/Desktop/RAT/targetFunctions/common/reflectivityCalculations/Abeles/abeles_paraPoints.m"/* pathName */
};

static emlrtRTEInfo cc_emlrtRTEI = { 42,/* lineNo */
  19,                                  /* colNo */
  "abeles_paraPoints",                 /* fName */
  "/home/sp1115144/Desktop/RAT/targetFunctions/common/reflectivityCalculations/Abeles/abeles_paraPoints.m"/* pName */
};
...
static emlrtRTEInfo sm_emlrtRTEI = { 9,/* lineNo */
  7,                                   /* colNo */
  "abeles_paraPoints",                 /* fName */
  "/home/sp1115144/Desktop/RAT/targetFunctions/common/reflectivityCalculations/Abeles/abeles_paraPoints.m"/* pName */
};
...
#pragma omp parallel \
 ...
 private(quo,num,MI,beta,pimag,preal,rough,thick,blast,psub,N,b_emlrtJBEnviron,b_st,i,preal_tmp,i1,reploop,re,nl,im,ar,d,quo_re,x_re,x_im,d1,i2,b_MI,brm,loop) \
...
#pragma omp for nowait
    ...
    for (loop = 0; loop <= ub_loop; loop++) {
      ...
      if (setjmp(b_emlrtJBEnviron) == 0) {
        i = loop + 1;
        if ((i < 1) || (i > x->size[0])) {
          emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &gk_emlrtBCI, &st);
        }
        ...
        blast.im = 0.0;
        if (1 > sld->size[0]) {
          emlrtDynamicBoundsCheckR2012b(1, 1, sld->size[0], &fk_emlrtBCI, &st);
        }
        ...
        for (reploop = 0; reploop < i; reploop++) {
          for (nl = 0; nl < i1; nl++) {
            ...
            for (i2 = 0; i2 < 2; i2++) {
              ...
              b_MI[i2 + 2].re = (im * ar - re * preal) + (brm * thick - quo_re *
                rough);
              b_MI[i2 + 2].im = (im * preal + re * ar) + (brm * rough + quo_re *
                thick);
            }
        ...
        for (i = 0; i < 2; i++) {
          ...
          b_MI[i + 2].re = (thick * ar - rough * preal) + (im * quo.re - re *
            quo.im);
          b_MI[i + 2].im = (thick * preal + rough * ar) + (im * quo.im + re *
            quo.re);
        }

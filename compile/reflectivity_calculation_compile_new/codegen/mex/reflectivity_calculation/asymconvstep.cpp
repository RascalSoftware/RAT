...
static emlrtRSInfo ec_emlrtRSI = { 40, // lineNo
  "asymconvstep",                      // fcnName
  "/home/arwel/Documents/RascalDev/RAT/targetFunctions/common/makeSLDProfiles/asymconvstep.m"// pathName 
};
                  ...
                  array<real_T, 2U> &f)
{
  ...
  b_x.set_size((&oh_emlrtRTEI), (&b_st), 1, x.size(1));
  ...
  st.site = &fc_emlrtRSI;
  b_st.site = &gc_emlrtRSI;
  b_x.set_size((&ph_emlrtRTEI), (&b_st), 1, x.size(1));
  loop_ub = x.size(0) * x.size(1);
  for (i = 0; i < loop_ub; i++) {
    b_x[i] = (x[i] - r) / bFactor;
  }
  ...
  r = h / 2.0;
  i = f.size(0) * f.size(1);
  f.set_size((&qh_emlrtRTEI), sp, 1, f.size(1));
  loop_ub = i - 1;
  for (i = 0; i <= loop_ub; i++) {
    f[i] = r * (f[i] - b_r[i]);
  }

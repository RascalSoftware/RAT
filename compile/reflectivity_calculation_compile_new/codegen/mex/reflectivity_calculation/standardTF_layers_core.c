...
static emlrtRTEInfo oh_emlrtRTEI = { 84,/* lineNo */
  47,                                  /* colNo */
...
void b_standardTF_layers_core(const emlrtStack *sp, const emxArray_real_T
  *contrastLayers, real_T rough, const emxArray_char_T *geometry, real_T nba,
  ...
  real_T simLimits_data[], const int32_T simLimits_size[2], const real_T
  repeatLayers[2], real_T background, real_T resol, real_T backsType, real_T
  params, const real_T resamPars[2], emxArray_real_T *sldProfile,
  ...
  real_T *ssubs)
{
  ...
  if (calcSld == 1.0) {
    st.site = &x_emlrtRSI;
    makeSLDProfiles(&st, nba, nbs, theseLayers, *ssubs, repeatLayers, sldProfile);
  } else {
    i = sldProfile->size[0] * sldProfile->size[1];
    sldProfile->size[0] = 1;
    sldProfile->size[1] = 2;
    emxEnsureCapacity_real_T(sp, sldProfile, i, &kh_emlrtRTEI);
  ...
  if (resample == 1.0) {
    ...
    emxEnsureCapacity_real_T(sp, resamLayers, i, &mh_emlrtRTEI);
    ...
    layerSld->size[1] = 3;
    emxEnsureCapacity_real_T(sp, layerSld, i, &lh_emlrtRTEI);
  ...
  if (backsType != (int32_T)muDoubleScalarFloor(backsType)) {
    emlrtIntegerCheckR2012b(backsType, &cb_emlrtDCI, &st);
  }
  ...
  switch ((int32_T)backsType) {
   ...
   case 2:
    ...
    emxInit_real_T(&st, &b_shifted_dat, 1, &qh_emlrtRTEI, true);

...
namespace RAT
{
  namespace nonPolarisedTF
  {
    static void c_contrastCalculation(real_T backgroundParamIndex, real_T
      ...
      const real_T resampleParams[2], boolean_T useImaginary, real_T roughness,
      const ::coder::array<real_T, 2U> &sldProfile, real_T *backgroundParamValue,
      ...
      &layerSld, ::coder::array<real_T, 2U> &resampledLayer);
  }
}
...
namespace RAT
{
  namespace nonPolarisedTF
  {
      ...
      &layerSld, ::coder::array<real_T, 2U> &resampledLayer)
    {
      ...
      //  Resample the layers
      if (!useImaginary) {
        resampleLayers(sldProfile, resampleParams, r);
        ...
        resampleLayersReIm(b_sldProfile, c_sldProfile, resampleParams, r);
                    ...
                    &resampledLayers, ::coder::array<real_T, 1U> &subRoughs)
    {
      ...
      customXY::processCustomFunction(problemStruct->contrastBulkIns,
        ...
        problemStruct->params, sldProfiles, subRoughs);
...
#pragma omp parallel for \
 ...
 private(r,r2,d,d1,d2,d3,d4,d5,d6,c_loop_ub,b_i,i2,d_loop_ub,i3)
        ...
        for (b_i = 0; b_i <= ub_loop; b_i++) {
          ...
          for (i2 = 0; i2 < c_loop_ub; i2++) {
            ...
            for (i3 = 0; i3 < d_loop_ub; i3++) {
              r2[i3 + r2.size(0) * i2] = sldProfiles[b_i].f1[i3 +
          ...
          c_contrastCalculation(problemStruct->contrastBackgrounds[b_i],
                                ...
                                subRoughs[b_i], r2, &d6, &d5, &d4, &d3, &d2, &d1,
                                &d, reflectivity[b_i].f1, simulation[b_i].f1, r,
          ...
          for (i2 = 0; i2 < c_loop_ub; i2++) {
            d_loop_ub = r.size(0);
            for (i3 = 0; i3 < d_loop_ub; i3++) {
              shiftedData[b_i].f1[i3 + shiftedData[b_i].f1.size(0) * i2] = r[i3
                + r.size(0) * i2];
            }
          }
        ...
        for (int32_T i{0}; i < ub_loop; i++) {
          ...
          r1.set_size(sldProfiles[i].f1.size(0), sldProfiles[i].f1.size(1));
          for (c_i = 0; c_i < loop_ub; c_i++) {
            b_loop_ub = sldProfiles[i].f1.size(0);
            for (i1 = 0; i1 < b_loop_ub; i1++) {
              r1[i1 + r1.size(0) * c_i] = sldProfiles[i].f1[i1 + sldProfiles[i].
                f1.size(0) * c_i];
            }
          }

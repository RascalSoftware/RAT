...
Fitting kinetic runs NR data - P60 1000 rpm (1) init. O3
'''
#%%
...
#    D2O_layer = D2O(5,2)
    ...
    film_layer.thick.setp(bounds=(350, 465), vary=True)
...
#    D2O_layer.rough.setp(bounds=(1,5),vary=True)
    ...
    MCMC = True
    constrain_sfs = True
    print('P60 1000 rpm, kinetic run init O3 (1)')
    
    if model_no == 1:
        # Make a multilayer by using a Stack Component
        stack = Stack(components=OA_layer|SO_layer, repeats=12)
        #stack |= OA_layer
        #stack |= SO_layer
        #stack.repeats = 10.0
...
#%%
    ...
    if constrain_sfs == True:
        output_df.to_csv('P60_1000rpm_initO3_kinetic_fitted_params_MCMC_GenAlg_constrainsfs_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    

    else:
        output_df.to_csv('P60_1000rpm_initO3_kinetic_fitted_params_MCMC_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'P60_1000rpm_initO3_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        if constrain_sfs == True:
            filename = 'P60_1000rpm_initO3_kinetic_nlc_MCMC_constrainsfs_model{}_run{}.pkl'.format(model_no,file_no)

        else:
            filename = 'P60_1000rpm_initO3_kinetic_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

...
Fitting kinetic runs NR data - P60 1000 rpm (4) reincRH
'''
#%%
...
for file in data_filenames_sorted:
    ...
    MCMC = True
    constrain_sfs = True
    print('P60 1000 rpm, kinetic run reincRH (4)')
...
#%%
    ...
    if constrain_sfs == True:
        output_df.to_csv('P60_1000rpm_reincRH_kinetic_fitted_params_MCMC_GenAlg_constrainsfs_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    

    else:
        output_df.to_csv('P60_1000rpm_reincRH_kinetic_fitted_params_MCMC_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'P60_1000rpm_reincRH_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        if constrain_sfs == True:
            filename = 'P60_1000rpm_reincRH_kinetic_MCMC_nlc_constrainsfs_model{}_run{}.pkl'.format(model_no,file_no)

        else:
            filename = 'P60_1000rpm_reincRH_kinetic_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

...
Fitting kinetic runs NR data - Disk A 2000 rpm (1) init. O3
'''
#%%
...
directory = r'C:\Users\Adam\DataShare\PhD\ISIS Sep 2019\oa-so-spincoating-NR-analysis\RDAaaS\NR_reduced_data_fitting\NR_reduced_data_fits\NR_reduced_data_fits\fig3_b_2000rpm\1_ozone_lowconc'
...
#    D2O_layer = D2O(5,2)
    ...
    film_layer.thick.setp(bounds=(300,346), vary=True)
...
#    D2O_layer.rough.setp(bounds=(1,5),vary=True)
    ...
    constrain_sfs = True
    if MCMC == True:
         print('DA 2000 rpm, kinetic run init O3 (1), MCMC sampling')
    else:
        print('DA 2000 rpm, kinetic run init O3 (1)')
    ...
    if model_no == 1:
        ...
        #stack |= SO_layer
        #stack.repeats = 10.0
        stack.repeats.setp(bounds=(7,9),vary=True) # vary repeats
...
#%%
    ...
    if MCMC == True:
        if constrain_sfs == True:
            output_df.to_csv('DA2000rpm_initO3_kinetic_fitted_params_GenAlg_constrainsfs_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    

        else:
            output_df.to_csv('DA2000rpm_initO3_kinetic_fitted_params_GenAlg_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
    else:
        output_df.to_csv('DA2000rpm_initO3_kinetic_fitted_params_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'DA2000rpm_initO3_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        if constrain_sfs == True:
            filename = 'DA2000rpm_initO3_kinetic_MCMC_nlc_constrainsfs_model{}_run{}.pkl'.format(model_no,file_no)

        else:
            filename = 'DA2000rpm_initO3_kinetic_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

...
for file in data_filenames_sorted:
    ...
    film_layer.thick.setp(bounds=(250, 380), vary=True)
    ...
    if MCMC == True:
        print('DA 2000 rpm, kinetic run inc. RH (3) MCMC sampling')
    else:
        print('DA 2000 rpm, kinetic run inc. RH (3)')
    ...
    if model_no == 1:
        ...
        stack = Stack(components= OA_layer|SO_layer|D2O_layer, repeats = 9)
        stack.repeats.setp(bounds=(6,10),vary=True) # vary repeats
...
#%%
    ...
    if MCMC == True:
        if constrain_sfs == True:
            output_df.to_csv('DA2000rpm_incRH_kinetic_fitted_params_GenAlg_constrainsfs_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
        else:
            output_df.to_csv('DA2000rpm_incRH_kinetic_fitted_params_GenAlg_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
    else:
        output_df.to_csv('DA2000rpm_incRH_kinetic_fitted_params_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'DA2000rpm_incRH_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        if constrain_sfs == True:
            filename = 'DA2000rpm_incRH_kinetic_MCMC_nlc_constrainsfs_model{}_run{}.pkl'.format(model_no,file_no)
        else:
            filename = 'DA2000rpm_incRH_kinetic_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

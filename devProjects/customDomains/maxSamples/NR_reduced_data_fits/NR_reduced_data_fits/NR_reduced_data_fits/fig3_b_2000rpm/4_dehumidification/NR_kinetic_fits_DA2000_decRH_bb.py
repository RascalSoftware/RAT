# -*- coding: utf-8 -*-
"""
Created on Mon Dec 21 13:47:22 2020

@author: Adam
"""

'''
Adam Milsom, University of Birmingham Dec 2020
...
for file in data_filenames_sorted:
    ...
    constrain_sfs = True
    if MCMC == True:
        print('DA 2000 rpm, kinetic run dec. RH (3) MCMC sampling')       
    else:
        print('DA 2000 rpm, kinetic run dec. RH (3)')
    ...
    if model_no == 1:
        ...
        stack = Stack(components= OA_layer|SO_layer|D2O_layer, repeats = 3)
        stack.repeats.setp(bounds=(2,8),vary=True) # vary repeats
        ...
        model = MixedReflectModel((structure1,structure2), scales=(0.7,0.1), bkg=3e-7, dq=2.5)
        model.scales[0].setp(bounds=(0.0, 1.2), vary=True)
        model.scales[1].setp(bounds=(0.0, 1.2), vary=True)
        model.bkg.setp(bounds=(1e-9, 9e-6), vary=True)
        model.threads = 1
...
#%%
    ...
    if MCMC == True:
        if constrain_sfs == True:
            output_df.to_csv('DA2000rpm_decRH_kinetic_fitted_params_GenAlg_constrainsfs_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    

        else:
            output_df.to_csv('DA2000rpm_decRH_kinetic_fitted_params_GenAlg_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
    else:
        output_df.to_csv('DA2000rpm_decRH_kinetic_fitted_params_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'DA2000rpm_decRH_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        if constrain_sfs == True:
            filename = 'DA2000rpm_decRH_kinetic_nlc_MCMC_constrainsfs_model{}_run{}.pkl'.format(model_no,file_no)

        else:
            filename = 'DA2000rpm_decRH_kinetic_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

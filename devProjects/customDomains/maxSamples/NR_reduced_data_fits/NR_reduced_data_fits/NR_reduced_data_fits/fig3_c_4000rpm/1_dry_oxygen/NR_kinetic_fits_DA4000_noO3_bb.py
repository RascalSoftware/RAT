...
Fitting kinetic runs NR data - Disk A 4000 rpm (1) no O3, dry init
'''
#%%
...
#    D2O_layer = D2O(5,2)
    ...
    OA_layer.sld.real.setp(bounds=(0.2, 5), vary=True)
    OA_layer.rough.setp(bounds=(1, 10), vary=True)
    
    SO_layer.thick.setp(bounds=(15, 25), vary=True)
    SO_layer.sld.real.setp(bounds=(0.2, 5), vary=True)
...
#    D2O_layer.rough.setp(bounds=(1,5),vary=True)
    ...
    MCMC = True
    if MCMC == True:
         print('DA 4000 rpm, kinetic run no O3 init (1), MCMC sampling')
    else:
        print('DA 4000 rpm, kinetic run no O3 init (1)')
...
#%%
    ...
    if MCMC == True:
        output_df.to_csv('DA4000rpm_noO3_kinetic_fitted_params_GenAlg_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
    else:
        output_df.to_csv('DA4000rpm_noO3_kinetic_fitted_params_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
    
    # pickle model and objective
    filename = 'DA4000rpm_noO3_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        filename = 'DA4000rpm_noO3_kinetic_nlc_constrainsfs_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

...
Fitting kinetic runs NR data - Disk A 4000 rpm (2) O3
'''
#%%
...
for filename in listdir(directory):
    if filename.endswith('.txt') and filename.find('corr') == -1:
        print(filename)
        data_filenames.append(filename)

# sort data filenames
run_nos = np.linspace(1,23,22).astype(int)
data_filenames_sorted = []
print('Data processing order: ')
for i in run_nos: 
    for name in data_filenames:
        if name.startswith('run_{}_'.format(i)): # format(i + 1) for "normal" runs
            data_filenames_sorted.append(name)
            print(name)

file_no = 0 # fitting just run 20 and 21
for file in data_filenames_sorted:
    ...
    # don't fit data where beam was down (NaNs recorded)
    # beam was down for some runs, these are ignored
    if True in np.isnan(data):
        ...
        np.savetxt(file[:-4]+'_corr.dat',corr_data[:,:3]) # only 1st 3 cols
    ...
    #    D2O_layer = D2O(5,2)
        ...
        film_layer.rough.setp(bounds=(20, 80), vary=True)#
    ...
    #    D2O_layer.rough.setp(bounds=(1,5),vary=True)
        ...
        MCMC = True
        if MCMC == True:
             print('DA 4000 rpm, kinetic run no O3 init (2), MCMC sampling')
        else:
            print('DA 4000 rpm, kinetic run no O3 init (2)')
    ...
    #%%
        ...
        if MCMC == True:
            output_df.to_csv('DA4000rpm_O3_kinetic_fitted_params_GenAlg_model{}_run{}_MCMC.csv'.format(model_no,file_no),index_label=False)    
        else:
            output_df.to_csv('DA4000rpm_O3_kinetic_fitted_params_GenAlg_model{}_run{}.csv'.format(model_no,file_no),index_label=False)    
        
        # pickle model and objective
        filename = 'DA4000rpm_O3_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
        if MCMC == True:
            filename = 'DA4000rpm_O3_kinetic_nlc_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

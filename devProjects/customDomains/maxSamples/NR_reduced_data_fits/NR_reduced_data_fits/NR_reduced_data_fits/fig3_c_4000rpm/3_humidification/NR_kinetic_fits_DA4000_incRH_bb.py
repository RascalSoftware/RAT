...
Fitting kinetic runs NR data - Disk A 4000 rpm (3) inc RH
'''
#%%
...
for file in data_filenames_sorted:
    ...
    MCMC = True
    if MCMC == True:
         print('DA 4000 rpm, kinetic run inc RH (3), MCMC sampling')
    else:
        print('DA 4000 rpm, kinetic run inc RH (3)')
    ...
    if model_no == 1:
        ...
        stack = Stack(components= OA_layer|SO_layer|D2O_layer, repeats = 6)
        stack.repeats.setp(bounds=(2,6),vary=True) # vary repeats
...
#%%
    ...
    filename = 'DA4000rpm_incRH_kinetic_model{}_run{}.pkl'.format(model_no,file_no)
    if MCMC == True:
        filename = 'DA4000rpm_incRH_kinetic_nlc_constrain_sfs_MCMC_model{}_run{}.pkl'.format(model_no,file_no)

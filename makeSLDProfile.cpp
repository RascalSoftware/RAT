...
namespace RAT
{
                      ...
                      &SLDProfile)
  {
    ...
    if (layers.size(0) > 0) {
      ...
      totalThickness = coder::sum(b_layers);
      b_layers.set_size(layers.size(0));
      loop_ub = layers.size(0);
      for (i = 0; i < loop_ub; i++) {
        b_layers[i] = layers[i];
      ...
      if (std::isnan(coder::sum(b_layers) + 150.0)) {
        x.set_size(1, 1);
        x[0] = rtNaN;
      } else if (totalThickness + 150.0 < 0.0) {
        x.set_size(1, 0);
      } else {
        x.set_size(1, static_cast<int>(totalThickness + 150.0) + 1);
        loop_ub = static_cast<int>(totalThickness + 150.0);
        for (i = 0; i <= loop_ub; i++) {
          x[i] = i;
        }
      }

      Lays.set_size(x.size(1), layers.size(0) + 2);
      loop_ub = layers.size(0) + 2;
      b_loop_ub = x.size(1);
      for (i = 0; i < loop_ub; i++) {
      ...
      for (int b_i{0}; b_i < i; b_i++) {
        ...
        double thisBoxCentre;
        layerThickness = layers[b_i];
        subsBoxCen_tmp = roughnessValues[b_i + 1];
        nextLayerRoughness = subsBoxCen_tmp;
        thisBoxCentre = lastBoxEdge + 0.5 * layerThickness;
        asymconvstep(x, layerThickness, thisBoxCentre, roughnessValues[b_i],
                     subsBoxCen_tmp, layers[b_i + layers.size(0)], r1);
        loop_ub = Lays.size(0);
        for (int i1{0}; i1 < loop_ub; i1++) {
          Lays[i1 + Lays.size(0) * b_i] = r1[i1];
        }
      ...
      asymconvstep(x, (subsBoxCen_tmp - lastBoxEdge) * 2.0, subsBoxCen_tmp,
                   nextLayerRoughness, ssub, bulkOut, r1);
      loop_ub = Lays.size(0);
      for (i = 0; i < loop_ub; i++) {
        Lays[i + Lays.size(0) * layers.size(0)] = r1[i];
      }
      ...
      loop_ub = Lays.size(0);
      for (i = 0; i < loop_ub; i++) {
        Lays[i + Lays.size(0) * (layers.size(0) + 1)] = r1[i];
      }

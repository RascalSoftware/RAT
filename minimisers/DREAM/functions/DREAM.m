...
for t = T_start : DREAMPar.nGenerations
    ...
    X(idx_ac,1:DREAMPar.nParams+2) = [xnew(idx_ac,1:DREAMPar.nParams) log_PR_xnew(idx_ac,1) log_L_xnew(idx_ac,1)]; fx(:,idx_ac) = fx_new(:,idx_ac);
    ...
    if mod(t,DREAMPar.steps) == 0
        ...
        if ( t <= DREAMPar.nGenerations / 10 )
            ...
            [X,log_L(1:t,2:DREAMPar.nChains+1),output.outlier] = removeOutlier(X,log_L(1:t,2:DREAMPar.nChains+1),output.outlier,DREAMPar,fid);
        end

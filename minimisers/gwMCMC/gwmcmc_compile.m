function [models,logP]=gwmcmc(minit,mccount,p,extras)
...
%         else %NON-PARALLEL
            for wix=1:Nwalkers
                ...
                if logrand(1,wix)<(numel(proposedm(:,wix))-1)*log(zz(wix))
                    for fix=1:NPfun
                        proposedlogP(fix)=logPFuns(fix, proposedm(:,wix),extras);
        ...
        textOut( ...
            (row-1+jj/p.ThinChain)/Nkeep, iteration, interval);%,sum(reject)/totcount)
    end
    models(:,:,row)=curm;
    logP(:,:,row)=curlogP;
...
textOut(100, iteration, interval);%,0);
if p.BurnIn>0
...
function out = logPFuns(which,in,extras)
...
function LogLike = logLike(x,extras)
...
function logPrior = logPrior(m,extras)
...
thisLogical = zeros(length(fitPars),1);

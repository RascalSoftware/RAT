function table = mainResolutionTableSetup(problem)

import javax.swing.table.DefaultTableModel;
import javax.swing.JTable;

headings = {'Fit?','Resolution Name','Lower Bound','Value','Upper Bound'};
...
constr = cell2mat(problemStruct.resolParConstr(:));
checks = problemStruct.resolFitYesNo;
names = problemStruct.resolParNames;
min = constr(:,1);
val = problemStruct.resolPars;
max = constr(:,2);
rows = length(val);
...
set(model,'TableChangedCallback',@mainGuiResolutionTableEditedCallback);

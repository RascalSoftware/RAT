...
namespace RAT
{
    ...
    double, 2U> &VEs, ::coder::array<double, 2U> &ns)
  {
    ...
    if ((nosplit != 0.0) || (static_cast<unsigned int>(u1.size(0)) < u.size(1) +
      ...
      coder::all((const boolean_T *)&empty_non_axis_sizes, (boolean_T *)
                 &tmp_data, VE_size);
      guard1 = false;
      if (coder::b_all((const boolean_T *)&tmp_data)) {
        guard1 = true;
      } else {
        empty_non_axis_sizes = (VE_data > 2.0 * VS);
        coder::all((const boolean_T *)&empty_non_axis_sizes, (boolean_T *)
                   &tmp_data, VE_size);
        if (coder::b_all((const boolean_T *)&tmp_data)) {
          guard1 = true;

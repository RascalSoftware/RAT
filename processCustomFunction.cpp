    ...
    namespace customLayers
    {
        ...
        array<double, 1U> &subRoughs)
      {
        ...
        for (int b_i{0}; b_i < i; b_i++) {
          ...
          if ((!std::isnan(x.re)) && (!std::isnan(x.im))) {
            ...
            callCppFunction((const char *)((::coder::array<char, 2U> *)
              &customFiles[static_cast<int>(d) - 1].f1)->data(), iv, params,
                            ...
                            thisContrastLayers, &subRoughs[b_i]);
          ...
          //  custom function).
          if (!useImaginary) {
            applyHydrationReal(b_thisContrastLayers, bulkInArray[static_cast<int>
                               (contrastBulkIns[b_i]) - 1], bulkOuts[b_i]);
          } else {
            applyHydrationImag(b_thisContrastLayers, bulkInArray[static_cast<int>
                               (contrastBulkIns[b_i]) - 1], bulkOuts[b_i]);
          }

function [qzshifts,scalefactors,bulkIns,bulkOuts,chis,reflectivity,...
    ...
    domainLayers,domainResampledLayers,subRoughs] = domainsReflectivity(problemStruct,controls)
    ...
    switch lower(problemStruct.modelType)
        ...
        case coderEnums.modelTypes.CustomLayers
            ...
            [contrastLayers,subRoughs] = domainsTF.processCustomFunction(...
             contrastBulkInIndices,contrastBulkOutIndices,bulkInValues,...
             bulkOutValues,contrastCustomFiles,numberOfContrasts,...
        ...
        case coderEnums.modelTypes.CustomXY
            ...
            [slds,subRoughs] = domainsTF.processCustomFunction(...
             contrastBulkInIndices,contrastBulkOutIndices,bulkInValues,...
             bulkOutValues,contrastCustomFiles,numberOfContrasts,...
             numberOfOutputColumns,customFiles,paramValues,useImaginary);
        ...
        if strcmpi(parallel, coderEnums.parallelOptions.Contrasts)
            ...
            parfor i = 1:numberOfContrasts
                ...
                [reflectivity{i},simulation{i},shiftedData{i},backgrounds{i},...
                 ...
                 dataIndices,inputResolution,inputBackground,...
                 contrastBackgroundActions{i},nParams,parallel,resampleMinAngle,...
                 resampleNPoints,subRoughs(i),slds1{i},slds2{i});
        ...
        if strcmpi(parallel, coderEnums.parallelOptions.Contrasts)
            ...
            parfor i = 1:numberOfContrasts
                ...
                [reflectivity{i},simulation{i},shiftedData{i},sldProfiles{i},...
                 layers{i},resampledLayers{i},chis(i)...
                 ] = contrastLayersCalculation(bulkIns(i),bulkOuts(i),...
                 domainRatios(i),inputShiftedData,simulationXData,dataIndices,...
                 inputResolution,inputBackground,repeatLayers(i),...
...
function [reflectivity,simulation,shiftedData,sldProfile,layers,...
    resampledLayers,chi] = contrastLayersCalculation(bulkIn,bulkOut,...
    domainRatio,shiftedData,simulationXData,dataIndices,resolution,...
    background,repeatLayers,backgroundAction,nParams,parallel,...
    ...
    [reflectivity1,simulation1,~,sldProfile1,layers1,resampledLayers1...
     ] = normalTF.coreLayersCalculation(contrastLayers1,roughness,geometry,...
     bulkIn,bulkOut,resample,calcSld,shiftedData,simulationXData,dataIndices,...
     repeatLayers,resolution,background,backgroundAction,parallel,...
     resampleMinAngle,resampleNPoints);

    [reflectivity2,simulation2,shiftedData,sldProfile2,layers2,resampledLayers2...
     ] = normalTF.coreLayersCalculation(contrastLayers2,roughness,geometry,...
     bulkIn,bulkOut,resample,calcSld,shiftedData,simulationXData,dataIndices,...
     repeatLayers,resolution,background,backgroundAction,parallel,...
     resampleMinAngle,resampleNPoints);
...
function [reflectivity,simulation,shiftedData,background,resolution,...
    sldProfile,layers,resampledLayers,chi] = contrastCustomXYCalculation(...
    bulkIn,bulkOut,domainRatio,shiftedData,simulationXData,dataIndices,...
    resolution,background,backgroundAction,nParams,parallel,...
    resampleMinAngle,resampleNPoints,roughness,sld1,sld2)
    ...
    [reflectivity1,simulation1,~] = applyBackgroundCorrection(...
        reflectivity1,simulation1,shiftedData,background,backgroundAction);
    [reflectivity2,simulation2,shiftedData] = applyBackgroundCorrection(...
        reflectivity2,simulation2,shiftedData,background,backgroundAction);

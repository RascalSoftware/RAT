function [outSsubs,backgroundParams,qzshifts,scalefactors,bulkIns,bulkOuts,...
    ...
    if strcmpi(parallel, coderEnums.parallelOptions.Contrasts)
        ...
        parfor i = 1:numberOfContrasts
            ...
            nParams,parallel,resamPars,useImaginary,sldProfiles{i},outSsubs(i));
...
function [backgroundParamValue,qzshiftValue,scalefactorValue,bulkInValue,...
    ...
    nParams,parallel,resamPars,useImaginary,sldProfile,ssub)
    ...
    [reflect,simul] = callReflectivity(bulkInValue,bulkOutValue,simLimits,repeatLayers,shiftedDat,layerSld,ssub,resolutionParamValue,parallel,reflectivityType,useImaginary);
    ...
    if dataPresent
        chi = chiSquared(shiftedDat,reflect,nParams);
    else
        chi = 0;
    end

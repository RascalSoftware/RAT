function [allLayers,allRoughs] =  loopCppCustomLayersWrapperXYContrasts(cBacks,cShifts,cScales,cNbas,cNbss,cRes,backs,...
...
%     strfunctionName = string(functionName);
    if coder.target('MEX')
        ...
        for i = 1:numberOfContrasts
            ...
            %coder.extrinsic('testDLL_mex');
            output = zeros(99999,2);
            sRough = 0.0;
            [output,sRough] = testDLL_mex(params,nba,nbs,i,cLibName,cfunctionName);
            
            output = reshape(output,2,99999)'; % convert to 3 x from top down and transpose 
            mask = (output >= 0) & (output <= 2.6e-40); % mask the zeros/extremely small positive useless nums or use ...

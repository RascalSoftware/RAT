...
hiVal = normalizeFunction(initialDomain(:),sldProfile,vectorizable);
...
function y = normalizeFunction(x, sldProfile,vectorizable)
...
% end
no_elements = numel(x);
no_outputs = 1; %abs(nargout(func)); %for anonymous functions nargout<0
...
cosCornerAngle = ...
  0.5 * (firstStepSquared + secondStepSquared - longStepSquared) ./ ...
  sqrt(firstStepSquared .* secondStepSquared);
cornerAngle = real(acos(complex(cosCornerAngle)));
end
...
function dataPoints = increaseSampling(dataPoints, segmentsToSplit, sldProfile)
...
newDataPoints(:,2) = normalizeFunction(newDataPoints(:,1),sldProfile);

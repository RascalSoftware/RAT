...
domainRatio = problemDef.domainRatio;
...
[allLayers,allRoughs] = customModelClass.processCustomLayersDomains(cBacks,cShifts,cScales,cNbas,cNbss,cRes,backs,...
...
for i = 1:numberOfContrasts
    ...
    thisContrastLayers1 = allLayers{layersCounter};
    thisContrastLayers2 = allLayers{layersCounter+1};
    ...
    % for future use. In this case, we also keep the relevant SLD profile
    % for each domain by combining them into single arrays..
    outSsubs(i) = thisSsubs;
    sldProfiles{i} = {sldProfile1, sldProfile2};
    reflectivity{i} = totReflect;
    Simulation{i} = totSimul;
    shifted_data{i} = shifted_dat;
    layerSlds{i} = {layerSld1, layerSld2};
    allLayers{i} = {resamLayers1, resamLayers2};

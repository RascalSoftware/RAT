                      ...
                      nrepeats, emxArray_real_T *out)
{
  ...
  if (numberOfLayers > 0.0) {
    ...
    emxEnsureCapacity((emxArray__common *)b_layers, i2, (int)sizeof(double));
    for (i2 = 0; i2 < vstride; i2++) {
      b_layers->data[i2] = layers->data[i2];
    }
    ...
    if (rtIsNaN(layerThicks)) {
      ...
      for (i2 = 0; i2 <= vstride; i2++) {
        x->data[x->size[0] * i2] = i2;
      }
    }

    emxInit_real_T(&Lays, 2);
    i2 = Lays->size[0] * Lays->size[1];
    Lays->size[0] = x->size[1];
    Lays->size[1] = (int)(numberOfLayers * nrepeats + 2.0);
    emxEnsureCapacity((emxArray__common *)Lays, i2, (int)sizeof(double));
    ...
    for (i2 = 0; i2 < vstride; i2++) {
      b_x->data[i2] = (x->data[i2] - -50.0) / layerThicks;
    }
    ...
    emxFree_real_T(&b_x);
    for (i2 = 0; i2 < vstride; i2++) {
      c_x->data[i2] = (x->data[i2] - 50.0) / bFactor;
    }
    ...
    while (mtmp <= (int)nrepeats - 1) {
      for (i = 0; i < (int)numberOfLayers; i++) {
        ...
        i2 = (int)((1.0 + (double)i) + numberOfLayers * ((1.0 + (double)mtmp) -
                    1.0)) - 1;
        vstride = r2->size[0];
        for (b_mtmp = 0; b_mtmp < vstride; b_mtmp++) {
          Lays->data[r2->data[b_mtmp] + Lays->size[0] * i2] = airBox->
            data[b_mtmp];
        }
    ...
    emxInit_real_T(&y, 2);
    i2 = (int)(numberOfLayers * nrepeats + 2.0) - 1;
    ...
    vstride = r2->size[0];
    for (b_mtmp = 0; b_mtmp < vstride; b_mtmp++) {
      Lays->data[r2->data[b_mtmp] + Lays->size[0] * i2] = y->data[b_mtmp];
    }
    ...
    emxEnsureCapacity((emxArray__common *)b_SLD, i2, (int)sizeof(double));
    ...
    emxFree_real_T(&Lays);
    i2 = SLD->size[0] * SLD->size[1];
    SLD->size[0] = b_SLD->size[0];
    SLD->size[1] = 1;
    emxEnsureCapacity((emxArray__common *)SLD, i2, (int)sizeof(double));
    vstride = b_SLD->size[0];
    for (i2 = 0; i2 < vstride; i2++) {
      SLD->data[i2] = b_SLD->data[i2];
    }
    ...
    b_asymconvstep(r0->data, b_mtmp, 0.0, ssub, ssub, nbair, subBox_data,
                   subBox_size);
    i2 = airBox->size[0] * airBox->size[1];
    airBox->size[0] = 1;
    airBox->size[1] = 101;
    emxEnsureCapacity((emxArray__common *)airBox, i2, (int)sizeof(double));
    emxFree_real_T(&r0);
    for (i2 = 0; i2 < 101; i2++) {
      airBox->data[i2] = subBox_data[i2];
    }
    ...
    emxFree_real_T(&r1);
    for (i2 = 0; i2 < vstride; i2++) {
      SLD->data[i2] = airBox->data[i2] + subBox_data[i2];
    }
  }
  ...
  i2 = out->size[0] * out->size[1];
  out->size[0] = vstride;
  out->size[1] = 2;
  emxEnsureCapacity((emxArray__common *)out, i2, (int)sizeof(double));
  for (i2 = 0; i2 < vstride; i2++) {
    out->data[i2] = x->data[i2];
  }

  emxFree_real_T(&x);
  for (i2 = 0; i2 < mtmp; i2++) {
    out->data[i2 + out->size[0]] = SLD->data[i2];
  }

...
static emlrtRTEInfo vb_emlrtRTEI = { 17,/* lineNo */
  5,                                   /* colNo */
  "chiSquared",                        /* fName */
  "/home/arwel/Documents/RascalDev/RAT/targetFunctions/common/chiSquared/chiSquared.m"/* pName */
};
                  ...
                  emxArray_real_T *thisFit, real_T P)
{
  ...
  emxInit_real_T1(sp, &n, 1, &vb_emlrtRTEI, true);
  i24 = thisData->size[0];
  mtmp = thisFit->size[0];
  if (i24 != mtmp) {
    emlrtSizeEqCheck1DR2012b(i24, mtmp, &h_emlrtECI, sp);
  }
  ...
  if (!overflow) {
    emlrtErrorWithMessageIdR2012b(&st, &oc_emlrtRTEI, "MATLAB:dimagree", 0);
  }

  emxInit_real_T1(&st, &a, 1, &sb_emlrtRTEI, true);
  i24 = a->size[0];
  a->size[0] = n->size[0];
  ...
  emxInit_real_T1(&st, &terms, 1, &ub_emlrtRTEI, true);
  st.site = &ld_emlrtRSI;
  ...
  emxEnsureCapacity(&d_st, (emxArray__common *)terms, i24, (int32_T)sizeof
                    (real_T), &tb_emlrtRTEI);
  if (!dimagree(terms, a)) {
    emlrtErrorWithMessageIdR2012b(&d_st, &pc_emlrtRTEI, "MATLAB:dimagree", 0);
  }
...
#pragma omp parallel for \
 num_threads(emlrtAllocRegionTLSs(c_st.tls, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs()))
  ...
  emxFree_real_T(&a);
  emxInit_boolean_T(&c_st, &x, 1, &sb_emlrtRTEI, true);
  st.site = &md_emlrtRSI;
  i24 = x->size[0];
  ...
  if (!(n->size[0] == 0)) {
    ...
    for (i24 = 0; i24 < nx; i24++) {
      ...
      if (!((mtmp >= 1) && (mtmp <= idx))) {
        emlrtDynamicBoundsCheckR2012b(mtmp, 1, idx, &df_emlrtBCI, sp);
      }

                  ...
                  *thisFit, double P)
{
  ...
  i15 = n->size[0];
  n->size[0] = mtmp;
  emxEnsureCapacity((emxArray__common *)n, i15, (int)sizeof(double));
  for (i15 = 0; i15 < mtmp; i15++) {
    n->data[i15] = (thisData->data[i15 + thisData->size[0]] - thisFit->data[i15
                    + thisFit->size[0]]) / thisData->data[i15 + (thisData->size
      [0] << 1)];
  }
  ...
  emxEnsureCapacity((emxArray__common *)terms, i15, (int)sizeof(double));
  mtmp = n->size[0];
...
#pragma omp parallel for \
 num_threads(omp_get_max_threads())
  ...
  mtmp = terms->size[0];
  for (i15 = 0; i15 < mtmp; i15++) {
    x->data[i15] = (terms->data[i15] == rtInf);
  }
  ...
  if (!(n->size[0] == 0)) {
    ...
    mtmp = n->size[0];
    for (i15 = 0; i15 < mtmp; i15++) {
      ii->data[i15] = (int)n->data[i15];
    }

    mtmp = ii->size[0];
    for (i15 = 0; i15 < mtmp; i15++) {
      terms->data[ii->data[i15] - 1] = 0.0;
    }
  }

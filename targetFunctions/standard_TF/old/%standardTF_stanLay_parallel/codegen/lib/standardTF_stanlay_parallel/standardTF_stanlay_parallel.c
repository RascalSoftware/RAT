...
void standardTF_stanlay_parallel(const emxArray_real_T *resam, double
  ...
  *repeatLayers, double nParams, const emxArray_real_T *params, const
  ...
  emxArray_cell_wrap_2 *layersDetails, double whichType, emxArray_real_T
  ...
  emxArray_cell_wrap_4 *allLayers, emxArray_real_T *allRoughs)
{
  ...
  double b_outLayers_data[10];
  int b_outLayers_size[2];
  double d1;
  double d2;
  double d3;
  double d4;
...
#pragma omp parallel \
 ...
 private(b_allData,layerSld,shifted_dat,reflect,sldProfile,d0,outLayers_data,outLayers_size,b_outLayers_data,b_outLayers_size,d1,d2,d3,d4,d5,loop_ub,i0)
...
#pragma omp for nowait
    ...
    for (i = 0; i <= ub_loop; i++) {
      ...
      nbss->data[i] = d1;
      resols->data[i] = d0;
      call_StandardLayers(params, contrastLayers->data[contrastLayers->size[0] *
                          i].f1, numberOfLayers, layersDetails, b_outLayers_data,
                          b_outLayers_size, &d0);
      allRoughs->data[i] = d0;
      allLayers->data[i].f1.size[0] = b_outLayers_size[0];
      allLayers->data[i].f1.size[1] = b_outLayers_size[1];
      loop_ub = b_outLayers_size[0] * b_outLayers_size[1];
      for (i0 = 0; i0 < loop_ub; i0++) {
        allLayers->data[i].f1.data[i0] = b_outLayers_data[i0];
      }

      groupLayers_Mod(b_outLayers_data, b_outLayers_size, allRoughs->data[i],
                      geometry, nbairs->data[i], nbsubs->data[i], outLayers_data,
                      outLayers_size, &d0);
      outSsubs->data[i] = d0;
      makeSLDProfiles(nbairs->data[i], nbsubs->data[i], outLayers_data,
                      outLayers_size, outSsubs->data[i], repeats->data
                      [repeats->size[0] * i].f1, sldProfile);
      ...
      if (resam->data[i] == 1.0) {
        ...
        i0 = layerSld->size[0] * layerSld->size[1];
        layerSld->size[0] = outLayers_size[0];
        layerSld->size[1] = 3;
        emxEnsureCapacity((emxArray__common *)layerSld, i0, (int)sizeof(double));
        loop_ub = outLayers_size[0] * outLayers_size[1];
        for (i0 = 0; i0 < loop_ub; i0++) {
          layerSld->data[i0] = outLayers_data[i0];
        }

        i0 = layerSlds->data[i].f1->size[0] * layerSlds->data[i].f1->size[1];
        layerSlds->data[i].f1->size[0] = outLayers_size[0];
        layerSlds->data[i].f1->size[1] = 3;
        emxEnsureCapacity((emxArray__common *)layerSlds->data[i].f1, i0, (int)
                          sizeof(double));
        loop_ub = outLayers_size[0] * outLayers_size[1];
        for (i0 = 0; i0 < loop_ub; i0++) {
          layerSlds->data[i].f1->data[i0] = outLayers_data[i0];
        }
      }

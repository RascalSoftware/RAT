function [output,sub_rough] = definedLength(params,bulk_in,bulk_out,contrast)
...
%thickTail = thickTail*cosd(theta);
thickTail = (1.5 + 16*1.265)*cosd(theta);

switch deb
    case 1
        if (thisMask(2)==0 && thisMask(1)==0); %H-Head ACMW
                addInfoText(sprintf('Hydr Head / ACMW - Unhydrated is %4.4d ',db_sldHead'));
                addInfoText(sprintf('Hydr Head / ACMW -  Hydrated is %4.4d ',sldHead'));
        elseif (thisMask(2)==1 && thisMask(1)==0) %D head acmw
                addInfoText(sprintf('Deut head / ACMW - Unhydrated is %4.4d ',db_sldHead'));
                addInfoText(sprintf('Deut head - ACMW - Hydrated is %4.4d ',sldHead'));
        elseif (thisMask(2)==0 && thisMask(1)==1) %H head D2O
                addInfoText(sprintf('Hydr head / D2O - Unhydrated is %4.4d ',db_sldHead'));
                addInfoText(sprintf('Hydr head - D2O - Hydrated is %4.4d ',sldHead'));
        elseif (thisMask(2)==1 && thisMask(1)==1) %D head D2O
                addInfoText(sprintf('Deut head / D2O - Unhydrated is %4.4d ',db_sldHead'));
                addInfoText(sprintf('Deut head - D2O - Hydrated is %4.4d ',sldHead'));
        end
end

             ...
             ::coder::array<real_T, 2U> &Vq)
{
  ...
  st.site = &p_emlrtRSI;
  y.set_size(&v_emlrtRTEI, &st, varargin_2.size(0));
  n = varargin_2.size(0);
  for (nd2 = 0; nd2 < n; nd2++) {
    y[nd2] = varargin_2[nd2];
  }
  x.set_size(&w_emlrtRTEI, &st, varargin_1.size(0));
  n = varargin_1.size(0);
  for (nd2 = 0; nd2 < n; nd2++) {
  ...
  Vq.set_size(&x_emlrtRTEI, &st, 1, varargin_3.size(1));
  n = varargin_3.size(1);
  for (nd2 = 0; nd2 < n; nd2++) {
    Vq[nd2] = rtNaN;
  }
        ...
        for (k = 2; k <= nx; k++) {
          if (x[k - 1] <= x[k - 2]) {
            emlrtErrorWithMessageIdR2018a(
                &st, &c_emlrtRTEI, "Coder:toolbox:interp1_nonMonotonicX",

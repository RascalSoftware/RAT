...
classdef testProjectClass < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testWriteScript(testCase)
            ...
            testCase.verifyError(@() testCase.project.writeScript(script="invalid.txt"), invalidValue.errorID)
            testCase.project.writeScript(objName="problem", script="projectScript");
            run("projectScript.m");
            ...
            for i=1:length(testCase.project.contrasts.contrasts)
                testCase.verifyEqual(testCase.project.contrasts.contrasts{i}, problem.contrasts.contrasts{i}, sprintf('contrast %i is not correctly regenerated by the script', i))
            end
